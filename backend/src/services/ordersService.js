...
export const getOrderById = async (id) => {
  const { rows } = await pool.query(
    `SELECT  orders.*,  clients.company_name, clients.contact_name  FROM orders INNER JOIN clients ON clients.id = orders.client_id WHERE orders.id = $1`,
    [id],
  );
  return rows[0] ?? null;
};
