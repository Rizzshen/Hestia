...
export const getIngredientsByProductId = async (productId) => {
  const { rows } = await pool.query(
    "SELECT  pi.id,  pi.product_id,  pi.quantity_needed,  rm.id AS raw_material_id,  rm.name,  rm.unit FROM product_ingredients pi INNER JOIN raw_materials rm  ON pi.raw_material_id = rm.id WHERE pi.product_id = $1",
    [productId],
  );
  return rows;
};
...
export const addIngredient = async (
  ...
  const { rows } = await pool.query(
    `INSERT INTO product_ingredients (product_id, raw_material_id, quantity_needed) VALUES ($1, $2, $3) RETURNING *`,
